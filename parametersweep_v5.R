...
compare_mutant_sequences <- function(motif_match_results) {
  ...
  both_mutated_mod <- if (!is.na(paired1_result$diff) && !is.na(paired2_result$diff)) {
    intersect_set <- intersect(
      setdiff(paired1_comp_seq, paired1_disr_seq),
      setdiff(paired2_comp_seq, paired2_disr_seq)
    )
    count_result <- filter_and_count(data.frame(filename = intersect_set), pool1_df)
    count_result$count
  } else NA
  ...
  return(list(
    ...
    both_mutated_mod = both_mutated_mod
  ))
}
...
define_all_possible_columns <- function() {
  ...
  comparison_columns <- c("paired1_disr_comp", "paired1_disr_comp_mod", "paired2_disr_comp", "paired2_disr_comp_mod", "both_mutated_mod")
