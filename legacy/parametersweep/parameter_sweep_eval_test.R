...
run_limited_combinations <- function() {
  ...
  for (unpaired1_min in 0:1) {
    for (unpaired1_max in 2:3) {
      for (paired1_min in 1:1) {
        for (paired1_max in 2:3) {
          for (unpaired2_min in 1:1) {
            for (unpaired2_max in 2:2) {
              ...
              # Test with paired2
              for (paired2_min in 1:1) {
                for (paired2_max in 2:2) {
                  result <- run_analysis(unpaired1_min, unpaired1_max, 
                                         paired1_min, paired1_max, 
                                         unpaired2_min, unpaired2_max, 
                                         paired2_min, paired2_max, 
  ...
  # Find top 10 parameter sets for incell and invitro
  top10_incell <- all_results[order(all_results$f1_incell, decreasing = TRUE), ][1:10, ]
  top10_invitro <- all_results[order(all_results$f1_invitro, decreasing = TRUE), ][1:10, ]
  
  cat("Top 10 parameters for incell:\n")
  print(top10_incell)
  cat("\nTop 10 parameters for invitro:\n")
  print(top10_invitro)
  
  # Save top 10 parameter sets
  write.csv(top10_incell, path(output_dir, "top10_parameters_incell.csv"), row.names = FALSE)
  write.csv(top10_invitro, path(output_dir, "top10_parameters_invitro.csv"), row.names = FALSE)
  
  return(list(incell = top10_incell, invitro = top10_invitro))
}
...
cat("2. Top 10 parameters for incell:", path(output_dir, "top10_parameters_incell.csv"), "\n")
cat("3. Top 10 parameters for invitro:", path(output_dir, "top10_parameters_invitro.csv"), "\n")
