...
#
# Arguments:
#   --pool1 <path>      Path to the annotated profile file for pool 1 data.
#   --pool2 <path>      Path to the annotated profile file for pool 2 data.
...
# --- Argument Parsing ---
parser <- ArgumentParser(description = "Compare SHAPE reactivity profiles between two data sets.")
parser$add_argument("--pool1", required = TRUE, help = "Annotated profile file for pool 1 data.")
parser$add_argument("--pool2", required = TRUE, help = "Annotated profile file for pool 2 data.")
...
dt1 <- fread(args$pool1)
dt2 <- fread(args$pool2)
...
# Merge the two data sets on matching RNA and nucleotide positions
merged_dt <- merge(dt1_good, dt2_good, by = c("RNA_name", "Nucleotide"), suffixes = c(".pool1", ".pool2"))
merged_dt <- na.omit(merged_dt) # Remove rows where a match wasn't found in both
cat(" -> Found", nrow(merged_dt), "matching high-quality nucleotide positions between data sets.\n")
...
generate_plot <- function(data, plot_title, filename) {
  ...
  corr <- cor(data$Norm_profile.pool1, data$Norm_profile.pool2, method = "pearson")
  corr_label <- paste0("Pearson's r = ", round(corr, 3))

  p <- ggplot(data, aes(x = Norm_profile.pool1, y = Norm_profile.pool2)) +
    geom_point(alpha = 0.2, shape = 16) +
    geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
    annotate("text", x = -Inf, y = Inf, label = corr_label, hjust = -0.2, vjust = 1.5, size = 5) +
    labs(
      title = plot_title,
      x = "SHAPE Reactivity (Pool 1 data)",
      y = "SHAPE Reactivity (Pool 2 data)"
    ) +
...
all_norm_values <- c(merged_dt$Norm_profile.pool1, merged_dt$Norm_profile.pool2)
...
non_outlier_dt <- merged_dt[
    (Norm_profile.pool1 >= lower_bound & Norm_profile.pool1 <= upper_bound) &
    (Norm_profile.pool2 >= lower_bound & Norm_profile.pool2 <= upper_bound)
]
