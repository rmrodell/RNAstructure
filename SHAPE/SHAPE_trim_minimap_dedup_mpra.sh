...
# This script uses minimap2 instead of bowtie for mapping.
#
...
usage() {
    echo "Usage: $0 -a <map-file> -i <input-dir> -o <output-dir> -r <ref-fasta>"
    echo ""
    echo "Options:"
    ...
    echo "  -r, --ref-fasta <path>    Path and base name for the Bowtie2 index."
    echo "  -h, --help                   Display this help message."
    exit 1
}

# Define short and long options
SHORT_OPTS="a:i:o:r:h"
LONG_OPTS="map-file:,input-dir:,output-dir:,ref-fasta:,help"
...
if [ -z "$SAMPLE_MAP_FILE" ] || [ -z "$BAM_SOURCE_DIR" ] || [ -z "$TOP_LEVEL_OUTPUT_DIR" ] || [ -z "$REF_FA" ]; then
...
if [ ! -f "$REF_FA" ]; then
    echo "Error: Reference fasta file not found. Checked for: $REF_FA" >&2; exit 1
fi
...
FINAL_COMMON_DIR="${TOP_LEVEL_OUTPUT_DIR}/final_deduplicated_bam"
...
FINAL_BAM_WITH_MD="${FINAL_DIR}/${SAMPLE_ID}_deduplicated_md.bam"
...
start_time=$(date +%s)
# samtools view -@ "$THREADS" -r "$MAPPED_SAM" | samtools sort -@ "$THREADS" -o "$MAPPED_BAM"
samtools sort -@ "$THREADS" -o "$MAPPED_BAM" "$MAPPED_SAM"
...
log_message "Step 9: Deduplicating reads with UMICollapse and indexing..."
start_time=$(date +%s)
...
# --- 11. Copy Final BAM to Common Output Directory ---
log_message "Step 11: Copying and verifying final BAM..."
start_time=$(date +%s)

DEST_BAM_PATH="${FINAL_COMMON_DIR}/${SAMPLE_ID}.bam"

# 1. Copy the file
cp "$FINAL_BAM_WITH_MD" "$DEST_BAM_PATH"
log_message "  - Copied to: ${DEST_BAM_PATH}"

# 2. Verify the copy with md5sum
log_message "  - Verifying file integrity with md5sum..."
source_md5=$(md5sum "$FINAL_BAM_WITH_MD" | awk '{print $1}')
...
# --- 13. Display Final Metrics Report ---
log_message "Step 13: Displaying run metrics summary for ${SAMPLE_ID}:"
