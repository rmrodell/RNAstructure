...
MAPPED_TAGGED_BAM="${TMP_DIR}/${SAMPLE_ID}_mapped_tagged.bam"
...
# --- Step 4: Add UMI to RX tag and convert to BAM ---
log_message "Step 4: Moving UMI from read name to RX tag..."
start_time=$(date +%s)
samtools view -h "$MAPPED_SAM" | \
    awk 'BEGIN{FS=OFS="\t"} /^@/{print; next} {split($1, a, "_"); $1=a[1]; for(i=2;i<length(a);i++){$1=$1"_"a[i]}; print $0"\tRX:Z:"a[length(a)]}' | \
    samtools view -@ "$THREADS" -b > "$MAPPED_TAGGED_BAM"
track_metrics "4_Tagged_BAM" "$MAPPED_TAGGED_BAM"
end_time=$(date +%s); log_message "Step 4 finished. Duration: $(format_duration $((end_time - start_time)))"

# --- Step 5: Sort and index the tagged BAM file ---
log_message "Step 5: Sorting UMI-tagged BAM..."
start_time=$(date +%s)
samtools sort -@ "$THREADS" -o "$MAPPED_BAM" "$MAPPED_TAGGED_BAM"
samtools index "$MAPPED_BAM"
track_metrics "5_Sorted_Tagged_BAM" "$MAPPED_BAM"
end_time=$(date +%s); log_message "Step 5 finished. Duration: $(format_duration $((end_time - start_time)))"

# --- Step 6: Deduplicate reads with GATK MarkDuplicates ---
log_message "Step 6: Deduplicating reads with GATK MarkDuplicates..."
start_time=$(date +%s)
GATK_METRICS_FILE="${LOGS_DIR}/${SAMPLE_ID}_gatk_dedup_metrics.txt"
