...
usage() {
    echo "Usage: $0 -u <email> -s <script> -a <map> -r <ref_index> -o <out_dir> [OPTIONS]..."
    echo ""
    ...
    echo "  -r, --ref-index <path>       Path to the directory containing the bowtie index."
    ...
    echo "      --ref-index /path/to/bowtie_index/ \\"
...
while [[ $# -gt 0 ]]; do
    ...
    case $key in
        ...
        -r|--ref-index)         REF_INDEX="$2"; shift 2 ;;
...
if [ -z "$REF_INDEX" ]; then missing_args+=("--ref_index"); fi
...
sbatch \
    ...
    "$PIPELINE_SCRIPT_PATH" -m "$SAMPLE_MAP_FILE" -r "$REF_INDEX" -o "$TOP_LEVEL_OUTPUT_DIR"
