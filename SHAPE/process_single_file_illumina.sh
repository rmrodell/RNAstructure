...
# --- Step 5: Deduplicate reads with UMICollapse ---
log_message "Step 5: Deduplicating reads with UMICollapse and indexing..."
start_time=$(date +%s)
...
track_metrics "5_Deduplicated_BAM" "$DEDUP_BAM"
end_time=$(date +%s); log_message "Step 5 finished. Duration: $(format_duration $((end_time - start_time)))"

# --- Step 6: Convert deduplicated BAM to final FASTQ for SHAPE-Mapper ---
log_message "Step 6: Converting deduplicated BAM to final FASTQ..."
start_time=$(date +%s)
samtools bam2fq -@ "$THREADS" "$DEDUP_BAM" > "$DEDUP_FASTQ"
track_metrics "6_Final_Deduplicated_FASTQ" "$DEDUP_FASTQ"
end_time=$(date +%s); log_message "Step 6 finished. Duration: $(format_duration $((end_time - start_time)))"

# --- Step 7: Copy final FASTQ to common output directory and verify ---
log_message "Step 7: Copying and verifying final FASTQ..."
start_time=$(date +%s)
...
log_message "Step 8: Cleaning up temporary directory..."
rm -rf "$TMP_DIR"

# --- Step 9: Display Final Metrics Report ---
log_message "Step 9: Displaying run metrics summary for ${SAMPLE_ID}:"
