...
option_list <- list(
  ...
  make_option(c("-p", "--psipos_file"), type = "character", help = "CSV file mapping sequence names to their psipos"),
  ...
  make_option("--max_unpaired1", type = "integer", default = 5),
  make_option("--min_paired1", type = "integer", default = 3),
  make_option("--max_paired1", type = "integer", default = 6),
  make_option("--min_unpaired2", type = "integer", default = 3),
  make_option("--max_unpaired2", type = "integer", default = 7),
...
psipos_file_path <- opt$psipos_file
...
cat("psipos_file_path:", psipos_file_path, "\n")
cat("fold_files_dir:", fold_files_dir, "\n")
cat("output_csv:", output_csv, "\n")
cat("\n")

# --- Load the psipos map ---
# Assumes columns are named 'name' and 'original_psipos'.
psipos_df <- read.csv(psipos_file_path, stringsAsFactors = FALSE)
psipos_map <- setNames(psipos_df$original_psipos, psipos_df$name)

# --- Read sequence IDs ---
# now pull the sequence ID from the psipos file
sequence_ids <- psipos_df$name
...
generate_result_list <- function(i, unpaired1_len, paired1_len, unpaired2_len, paired2_len, include_unpaired1, include_paired2) {
  ...
  prefix_len2 <- prefix_len1 + paired1_len
  prefix_len3 <- prefix_len2 + unpaired2_len

  result <- list(
    # Paired 1
    paired1_start = i + prefix_len1,
    paired1_end = i + prefix_len1 + paired1_len - 1, # Corrected
    paired1_length = paired1_len,
    # Unpaired 2
    unpaired2_start = i + prefix_len2,
    unpaired2_end = i + prefix_len2 + unpaired2_len - 1, # Corrected
  ...
  if (include_paired2) {
    result <- c(result, list(
      paired2_start = i + prefix_len3,
      paired2_end = i + prefix_len3 + paired2_len - 1, # Corrected
...
has_motif_at <- function(dot_bracket, i, min_unpaired1, max_unpaired1, min_paired1, max_paired1, 
  ...
  if (include_unpaired1 && unpaired1_len > 0) { # Check for length > 0
...
for (file in filtered_files) {
  ...
  if (length(lines) >= 3) {
    header <- sub(">", "", lines[1], fixed = TRUE)
    fasta_seq <- lines[2]
    dot_bracket <- strsplit(lines[3], " ")[[1]][1]

    input_position <- psipos_map[[header]]
    if (is.null(input_position)) {
      warning(paste("Could not find psipos for sequence:", header, "in the provided map. Skipping file:", basename(file)))
      next
    }
