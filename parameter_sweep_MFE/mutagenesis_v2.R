...
cat("Will protect -2, 0, 1, 2 from mutation")
...
option_list <- list(
  ...
  make_option("--protect_position", type = "integer", default = 60,
              help = "Central 1-based position to exclude from mutation. The script will also protect pos-2, pos+1, and pos+2. [default = %default]")
)
...
protect_pos_central <- opt$protect_position
...
# --- Calculate all positions to protect ---
# The script will protect the central position, plus pos-2, pos+1, and pos+2
protect_positions <- c(
    protect_pos_central - 2, 
    protect_pos_central, 
    protect_pos_central + 1, 
    protect_pos_central + 2
) # <-- ADDED
# Remove any non-positive values
protect_positions <- unique(protect_positions[protect_positions > 0])
cat("Protecting the following positions from mutation:", paste(protect_positions, collapse=", "), "\n")
...
generate_mutations <- function(seq_vec, pairs, start, end, protect_pos) {
  ...
  indices <- start:end
    indices <- indices[!indices %in% protect_positions]
  pair_indices <- pairs[indices]
  valid_indices <- indices[pairs[indices] != 0 & !pairs[indices] %in% protect_positions]
