...
# of a user-defined 5-nucleotide target region. It can change a paired region
...
option_list <- list(
  ...
  make_option(c("-t", "--target_pos"), type = "integer", default = NULL, 
              help = "Required. The 1-based CENTER of a 5-nucleotide block to flip (+/- 2).", metavar = "INT")
)

# Create the parser and parse the arguments.
opt_parser <- OptionParser(option_list = option_list, description = "Mutates RNA to flip the pairing status of a 5-mer region.")
...
CONFIG <- list(
  # Define the 5-nucleotide target region based on the user-provided center position.
  target_positions = (opt$target_pos - 2):(opt$target_pos + 2),
...
attempt_extend_stem <- function(org_data, config) {
  ...
  SEARCH_WINDOW <- 10 # How far (in nucleotides) to look for a nearby stem.
...
mutate_to_create_hairpin <- function(org_data, config) {
  ...
  return(list(
    sequence = paste(seq_vec, collapse = ""),
    info_string = paste(mutation_infos, collapse = ", "),
    strategy_used = "seed_hairpin_5bp"
  ))
}
...
main_workflow <- function(config) {
    ...
    mutate(
      is_successful = case_when(
        ...
        (original_status == "all_unpaired" | original_status == "mixed") & 
          ...
          strategy %in% c("extend_stem", "seed_hairpin_5bp") ~ TRUE,
  ...
  summary_text <- c(
    "===== 5-mer Pairing Status Flip: Summary Report =====",
    "",
