...
option_list <- list(
  ...
  make_option(c("-w", "--window"), type = "integer", default = 20,
              help = "Length of the downstream window (starting from psipos + 1) to look for base pairs. [default: %default]", metavar = "INT"),
...
# }
format_locations <- function(pairs_to_mutate, mutations, original_sequence, mutated_sequence) {
  if (length(pairs_to_mutate) == 0 || length(mutations) == 0) {
    return(list(basepair = "NA", mutation = "NA"))
  }
  
  basepair_str <- paste(sapply(pairs_to_mutate, function(pair) paste(pair, collapse = ":")), collapse = ", ")
  
  mutation_str <- paste(sapply(mutations, function(pos) {
    original_base <- substr(original_sequence, pos, pos)
    mutated_base <- substr(mutated_sequence, pos, pos)
    sprintf("%s%d%s", original_base, pos, mutated_base)
  }), collapse = ", ")
...
process_fold_file <- function(file_path, config) {
  ...
  locations <- format_locations(pairs_to_mutate, mutation_result$mutations, fold_data$seq, mutation_result$sequence)
